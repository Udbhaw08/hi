...
FACE_WEIGHT = float(os.getenv('FT_FACE_WEIGHT', '0.60'))
BODY_WEIGHT = float(os.getenv('FT_BODY_WEIGHT', '0.25'))
CLOTHING_WEIGHT = float(os.getenv('FT_CLOTHING_WEIGHT', '0.15'))

# Minimum face score required — prevents false matches from body/clothing alone
MIN_FACE_SCORE = float(os.getenv('FT_MIN_FACE_SCORE', '0.25'))

MATCH_THRESHOLD = float(os.getenv('FT_MATCH_THRESHOLD', '0.55'))
# Higher bar when all three modalities agree
CONFIDENT_THRESHOLD = float(os.getenv('FT_CONFIDENT_THRESHOLD', '0.65'))
SAVE_TOP_N = int(os.getenv('FT_SAVE_TOP_N', '10'))
MAX_SEARCH_SECONDS = int(os.getenv('FT_MAX_SEARCH_SEC', '300'))
# If True: a face MUST be detected in the frame to count as a valid match.
# Prevents body/clothing alone from matching any random person.
REQUIRE_FACE = os.getenv('FT_REQUIRE_FACE', '1') == '1'
...
class FaceTracer:
    ...
    def _score(self, face_emb=None, body_emb=None, clothing_feat=None):
        ...
        # Gate: if face was detected but score is too low, reject the match
        # (prevents body+clothing from falsely matching a different person)
